...
- **Labels & tooltips** — chips and rail pills show `name` (keep it short); the
  fuller `description` is the hover tooltip. Both are existing columns, editable
  from the bot and the web, so there's no dedicated "short label" field. The
  schedule API returns `name` + `description` per occurrence/suggestion. Long
...
1. Run migrations `030_exercise_queue.sql`, `031_exercise_schedule.sql`,
   and `032_exercise_anchor_date.sql` in Supabase.
