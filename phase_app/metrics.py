...
def get_bench_top_set_e1rm(conn: sqlite3.Connection, session_id: int) -> dict[str, Any] | None:
    row = conn.execute(
        """
        SELECT s.session_id, s.phase_id, s.session_date,
               ...
               es.reps,
               es.load_kg,
               ROUND(es.load_kg * (1 + es.reps / 30.0), 2) AS top_set_e1rm_kg
        FROM sessions s
    ...
    return {
        ...
        "topSetE1rmKg": row["top_set_e1rm_kg"],
        "aggregationVersion": None,
        "source": "live",
    }


def get_bench_volume(conn: sqlite3.Connection, session_id: int) -> dict[str, Any] | None:
    row = conn.execute(
        """
        SELECT s.session_id, s.phase_id, s.session_date,
               ROUND(COALESCE(SUM(es.load_kg * es.reps), 0), 2) AS bench_volume_kg_reps
        FROM sessions s
        JOIN session_exercises se ON se.session_id = s.session_id
        JOIN exercises e ON e.exercise_id = se.exercise_id
        JOIN exercise_sets es ON es.session_exercise_id = se.session_exercise_id
    ...
    return {
        ...
        "phaseId": row["phase_id"],
        "sessionDate": row["session_date"],
        "benchVolumeKgReps": row["bench_volume_kg_reps"],
        "aggregationVersion": None,
        "source": "live",
    }
