...
def _send_t(cur, conn, chat_id: int, text: str,
            reply_markup: dict | None = None) -> dict | None:
    ...
    The record is committed here rather than left to the caller. Most of these
    sends sit on a `return` in the webhook that never commits, so the row would
    be written and then lost — and a lost row is worse than none, because the
    sweep goes on looking like it works while quietly skipping the message.
    Pass conn=None only where the caller commits right after (the _cmd_*
    helpers, which don't take a connection).
    """
...
def _answer(callback_id: str, text: str = "", alert: bool = False) -> None:
    """Answer a tap. A toast by default; an alert when it has to be read.

    Telegram's toast is one short line that fades on its own, and a longer one
    is silently clipped — a tester was refused with a sentence she could only
    see the start of, and no way to expand it. Anything that says what to do
    next goes up as an alert, which stays until dismissed.
    """
    payload = {"callback_query_id": callback_id, "text": text}
    if alert:
        payload["show_alert"] = True
    _api_call("answerCallbackQuery", payload)
...
_STRINGS: dict[str, dict[str, str]] = {
    ...
    # Not "recorded", which is why people think they are done. Nothing has left
    # this chat yet, and the prompt is the only thing that can say so.
    "pick_prompt": {
        "en": "📹 Video ready. Who should it go to?",
        "uk": "📹 Відео готове. Кому надіслати?",
        "de": "📹 Video fertig. An wen soll es gehen?",
    },
...
def _main_kb(lang: str = "en", tg_id: int | None = None, cur=None) -> dict:
    ...
    is the only way to have both: a message can carry exactly one reply_markup,
    so the move confirmation can't hold Undo, the radar toggle *and* a labelled
    input. The keyboard lives in the client until it's re-sent, so its
...
_RADAR_INTRO = 3                      # radar deliveries that explain the setting
...
def _send_note(cur, conn, tg_id: int, chat_id: int, lang: str,
    ...
    owner = cur.fetchone()
    if not owner:
        _send_t(cur, conn, chat_id, _t("note_gone", lang))       # move was undone meanwhile
        return
...
def _revoke(cur, conn, tg_id: int, chat_id: int, lang: str, entry_id: int | None = None) -> bool:
    ...
    if age > _DELETE_LIMIT_HOURS * 3600:
        ...
        _send_t(cur, conn, chat_id, _t("undo_too_old", lang, hours=_DELETE_LIMIT_HOURS))
        return False
    ...
    conn.commit()
    _send_t(cur, conn, chat_id, _t("undo_done", lang), reply_markup=_main_kb(lang, tg_id, cur))
    return True
...
def _used_today(cur, tg_id: int, day=None) -> tuple[bool, set]:
    """What a day has already been spent on: the crew, and which circles.
    ...
    `day` defaults to today, but a move held overnight is asked about the day it
    belongs to: a pending entry from yesterday competes with yesterday's crew
    move, not with today's, and flushing it against the wrong day is how it came
    to violate the one-a-day index instead of falling back.
    """
    cur.execute("SELECT COALESCE(BOOL_OR(is_crew_wide), FALSE) AS w FROM move_entries "
                "WHERE telegram_user_id = %s AND entry_date = COALESCE(%s, CURRENT_DATE) "
                "  AND pending_since IS NULL", (tg_id, day))
    crew_used = bool((cur.fetchone() or {}).get("w"))
    cur.execute("SELECT circle_id FROM move_entry_circles "
                "WHERE owner_tg_id = %s AND entry_date = COALESCE(%s, CURRENT_DATE)",
                (tg_id, day))
...
def _fallback_audience(cur, tg_id: int, entry_id: int) -> tuple[set | None, bool]:
    ...
    cur.execute("SELECT entry_date FROM move_entries WHERE id = %s", (entry_id,))
    row = cur.fetchone()
    if not row:
        return None, False
    crew_used, used = _used_today(cur, tg_id, row["entry_date"])
    if not crew_used:
        return None, True
...
def announce_update(conn) -> None:
    ...
    for u in cur.fetchall():
        ...
        # Marked before the send, like the circles notice: a Telegram failure
        # must not turn into the same announcement every morning.
...
def _flush_one(cur, conn, tg_id: int, chat_id: int, entry_id: int, lang: str) -> None:
    """Send a move that was recorded and never addressed, or shelve it.

    Both callers went through here after the webhook's copy of this was found
    still doing what the job had been fixed not to do: falling back to the whole
    crew on a day the crew had already had one, which the one-a-day index
    refuses. Every message that person sent crashed on it before doing anything
    else, for a day and a half.
    """
    _drop_picker(cur, entry_id)
    audience, sendable = _fallback_audience(cur, tg_id, entry_id)
    if not sendable:
        # Nowhere left. Take it out of the pending state so it stops being
        # retried, and say so rather than leaving a video in limbo.
...
def flush_pending_moves(conn) -> None:
    ...
    for e in cur.fetchall():
        ...
        _flush_one(cur, conn, tg_id, chat_id, e["id"], lang)
...
def _cmd_start(cur, conn, tg_id: int, chat_id: int, lang: str, payload: str = "") -> None:
    ...
    if u and u["participant_name"]:
        if inviter_id is not None:
            ...
            _send_t(cur, conn, chat_id, _t("already_registered", lang, name=u["participant_name"]),
                  reply_markup=_main_kb(lang, tg_id, cur))
        return
...
def _handle_crew_name(cur, conn, tg_id: int, chat_id: int, lang: str, name: str) -> None:
    ...
    if not target or target["telegram_user_id"] == tg_id:
        ...
        me = _user(cur, tg_id)
        _send_t(cur, conn, chat_id, _t("crew_not_found", lang, name=name))
        return
    ...
    _send_t(cur, conn, chat_id, _t("crew_request_sent", lang, name=tname))


# ── introductions ───────────────────────────────────────────────
#
# Radar finds strangers with a query. This finds them with a person: somebody
# who knows both sides and can see what no cooldown table ever will.
#
# One asker, one decider — the same shape _apply_invite was rewritten into. Two
# symmetric requests would leave nobody in either role: accept first and you are
# waiting on something you cannot chase, and a decline becomes a social act with
# a witness rather than a button nobody sees you press. So a suggestion is a
# pre-filled invite handed to one person, who then sends the ordinary crew
# request. Everything downstream of that is the existing accept path.
#
# The suggester is told it went out and nothing else, ever. That is the whole
# reason a no stays cheap, and it will be the tempting thing to drop.

def _are_crew(cur, a_id: int, b_id: int) -> bool:
    """Does a already have b? move_crew keys on the name, so this resolves it."""
    b = _user(cur, b_id)
    if not b or not b["participant_name"]:
        return False
    cur.execute(
        "SELECT 1 FROM move_crew WHERE telegram_user_id = %s AND LOWER(crew_name) = LOWER(%s)",
...
def _intro_links(cur, ids: list[int]) -> set:
    """Every pair among `ids` that is already crew, or already introduced.

    Built in two queries rather than one per pair — twenty people is a hundred
    and ninety pairs, and this is on the path of a menu that redraws.
    """
    seen = set()
    if not ids:
        return seen
    cur.execute(
        "SELECT c.telegram_user_id AS a, u.telegram_user_id AS b "
        "FROM move_crew c JOIN move_users u ON LOWER(u.participant_name) = LOWER(c.crew_name) "
        "WHERE c.telegram_user_id = ANY(%s) AND u.telegram_user_id = ANY(%s)",
        (ids, ids),
    )
    for r in cur.fetchall():
        seen.add((min(r["a"], r["b"]), max(r["a"], r["b"])))
    cur.execute("SELECT a_id, b_id FROM move_intros "
                "WHERE a_id = ANY(%s) AND b_id = ANY(%s)", (ids, ids))
    for r in cur.fetchall():
        seen.add((min(r["a_id"], r["b_id"]), max(r["a_id"], r["b_id"])))
    return seen


def _intro_pairs(cur, tg_id: int) -> list:
    """Every (a, b) from my crew who could still be introduced, a before b."""
    people = _introducible(cur, tg_id)
    ids = [m["telegram_user_id"] for m in people]
    taken = _intro_links(cur, ids)
    return [(a, b) for i, a in enumerate(people) for b in people[i + 1:]
            if (min(a["telegram_user_id"], b["telegram_user_id"]),
                max(a["telegram_user_id"], b["telegram_user_id"])) not in taken]


def _intro_done(cur, a_id: int, b_id: int) -> bool:
    """Has this pair been introduced — by anyone, in either direction?"""
    cur.execute(
        "SELECT 1 FROM move_intros WHERE LEAST(a_id, b_id) = %s AND GREATEST(a_id, b_id) = %s",
        (min(a_id, b_id), max(a_id, b_id)),
    )
    return cur.fetchone() is not None


def _intro_partners(cur, tg_id: int, a_id: int) -> list:
    """Who a could still be introduced to, out of my crew."""
    out = []
    for a, b in _intro_pairs(cur, tg_id):
        if a["telegram_user_id"] == a_id:
            out.append(b)
        elif b["telegram_user_id"] == a_id:
            out.append(a)
    return out
...
def _intro_make(cur, conn, tg_id: int, a_id: int, b_id: int) -> int | None:
    ...
    if _are_crew(cur, a_id, b_id) or _intro_done(cur, a_id, b_id):
        return None
    cur.execute(
        "INSERT INTO move_intros (suggested_by, a_id, b_id) VALUES (%s, %s, %s) "
...
def _cmd_invite(cur, conn, tg_id: int, chat_id: int, lang: str) -> None:
    me = _user(cur, tg_id)
...
def _apply_invite(cur, conn, tg_id: int, chat_id: int, lang: str, inviter_id: int) -> None:
    ...
    """
    if inviter_id == tg_id:
        _send_t(cur, conn, chat_id, _t("invite_self", lang))
        return
...
def _radar_deliver(cur, conn, rid: int, chat_id: int, lang: str, cand,
    ...
    cur.execute("INSERT INTO move_radar_history (telegram_user_id, from_tg_id) VALUES (%s, %s)",
                (rid, cand["from_id"]))
    cur.execute("SELECT radar_freq, radar_intro, gender FROM move_users "
                "WHERE telegram_user_id = %s", (rid,))
    row = cur.fetchone() or {}
    explain = (row.get("radar_intro") or 0) < _RADAR_INTRO
    if explain:
        cur.execute("UPDATE move_users SET radar_intro = radar_intro + 1 "
                    "WHERE telegram_user_id = %s", (rid,))
    if touch_schedule:
    ...
    if explain:
        body += "\n\n" + _tgen("radar_first", lang, row.get("gender"),
...
def _cmd_radar(cur, conn, tg_id: int, chat_id: int, lang: str) -> None:
...
def _cmd_pause(cur, conn, tg_id: int, chat_id: int, lang: str) -> None:
...
def _cmd_mod(cur, conn, tg_id: int, chat_id: int, lang: str, args: str) -> None:
    ...
    if tg_id not in _admin_ids():
        ...
        _send_t(cur, conn, chat_id, _t("unknown_msg", lang), reply_markup=_main_kb(lang, tg_id, cur))
        return
    action, _, name = args.strip().partition(" ")
    name = name.strip()
    if action not in ("status", "restore", "ban") or not name:
        _send_t(cur, conn, chat_id, "Usage:\n/mod status <name>\n/mod restore <name>\n/mod ban <name>")
        return
    who = _by_name(cur, name)
    if not who:
        _send_t(cur, conn, chat_id, f"No one named \"{name}\".")
        return
...
def _handle_move_webhook(body: dict, conn) -> None:
    ...
    if base_state in ("await_name", "await_rename") and not text.startswith("/"):
        if not _valid_name(text):
            _send_t(cur, conn, chat_id, _t("letters_only", lang))
            return
        if _by_name(cur, text):
            _send_t(cur, conn, chat_id, _t("name_taken", lang, name=text))
            return
        cur.execute(
    ...
    if replied and not text.startswith("/"):
        ...
        if (msg.get("reply_to_message") or {}).get("from", {}).get("is_bot"):
            ...
            _send_t(cur, conn, chat_id, _t("reply_stale", lang))
            return
    ...
    if base_state == "await_feedback":
        ...
        if not text.startswith("/"):
            ...
            _send_t(cur, conn, chat_id, _t("feedback_sent", lang), reply_markup=_main_kb(lang, tg_id, cur))
            return
    ...
    if word in ("feedback", "support"):
        ...
        return
    if word == "radar":
        _cmd_radar(cur, conn, tg_id, chat_id, lang)
    ...
    if word == "settings":
        ...
        return
    if word == "pause":
        _cmd_pause(cur, conn, tg_id, chat_id, lang)
        return
    if word == "summary":
        _cmd_summary(cur, conn, tg_id, chat_id, lang)
        return
    if word == "invite":
    ...
    if word in ("language", "lang"):
        _send_t(cur, conn, chat_id, _LANG_PROMPT, reply_markup=_kb_lang())
        conn.commit()
        return
    ...
    if word == "log":
        if not args:
            _send_t(cur, conn, chat_id, _t("log_usage", lang))
            return
    ...
    _send_t(cur, conn, chat_id, _t("unknown_msg_done" if cur.fetchone() else "unknown_msg", lang),
          reply_markup=_main_kb(lang, tg_id, cur))
    conn.commit()
...
def _handle_callback(cur, conn, cq: dict) -> None:
    ...
    if body == "invite:rotate":
        ...
        _send_t(cur, conn, chat_id, _t("invite_rotated", lang, link=_invite_link(cur, tg_id)),
    ...
    if body.startswith("pk:"):
        ...
        if act == "c":
            ...
            if audience is None and _used_today(cur, tg_id)[0]:
                ...
                # so say that and nothing else. Telling her that everyone had
                # already had a move read as a rejection of the circle she
                # thought she had chosen.
                _answer(cq["id"], _t("pick_none", lang), alert=True)
    ...
    if body.startswith("cundo:"):
        ...
        _answer(cq["id"])
        _send_t(cur, conn, chat_id, _t("comment_undone", lang), reply_markup=_main_kb(lang, tg_id, cur))
        u = _user(cur, tg_id)
        return
    ...
    if body.startswith("intro:"):
        ...
        if sub.startswith("b:"):
            ...
            _answer(cq["id"])
            return
        if sub.startswith("go:"):
            a_s, _, b_s = sub[3:].partition(":")
            b = _user(cur, int(b_s))
            made = _intro_make(cur, conn, tg_id, int(a_s), int(b_s))
            a = _user(cur, int(a_s))
            if not made:
                # Re-checked at the last moment, so this is reachable: the two
                # may have connected on their own since the buttons were drawn.
            ...
            _log("\U0001fac2 Move: introduction suggested"
                 ...
                 + f"{b['participant_name'] if b else b_s}")
            _answer(cq["id"])
            return
        _answer(cq["id"])
        return
    ...
    if body.startswith("crew:"):
        ...
        if action == "addback":
            ...
            conn.commit()
            _send_t(cur, conn, chat_id, _t("crew_added_back", lang, name=name))
            me = _user(cur, tg_id)
            return
        ...
        if action == "removeok":
            ...
            _send_t(cur, conn, chat_id, _t("crew_removed", lang, name=tname))
            return
    ...
    if body == "radarnow":
        ...
        if not _radar_show(cur, conn, tg_id, chat_id, lang, touch_schedule=False):
            _send_t(cur, conn, chat_id, _t("radar_pull_none", lang))
            return
        u = _user(cur, tg_id)
        return
    ...
    if body.startswith("rblock:"):
        ...
        _api_call("editMessageReplyMarkup",
                  {"chat_id": chat_id, "message_id": msg_id, "reply_markup": {}})
        _answer(cq["id"])
        _send_t(cur, conn, chat_id, _t("radar_blocked", lang))
        u = _user(cur, tg_id)
        return
    ...
    if body.startswith("rreport:"):
        ...
        _api_call("editMessageReplyMarkup",
                  {"chat_id": chat_id, "message_id": msg_id, "reply_markup": {}})
        _answer(cq["id"])
        _send_t(cur, conn, chat_id, _t("radar_reported", lang))
        me = _user(cur, tg_id)
    ...
    if body.startswith("radarsend:"):
        ...
        if on and (_is_suspended(cur, tg_id) or _banned(u)):
            _answer(cq["id"])
            _send_t(cur, conn, chat_id, _t("radar_share_locked", lang))
            return
...
def offer_intros(conn) -> None:
    """Morning: tell anyone who has just become able to introduce two people.

    The webhook fires this too, on the accept that makes a link — but only for
    the two people in that link, and only from then on. A pair can appear for
    reasons no webhook sees: a warning expiring, somebody switching
    introductions back on, or simply having had the pair all along, since
    everyone did on the day this shipped. One pass a morning catches all of it.

    Guarded by intro_hinted_at rather than cron_log: the job is per-person and
    says one thing once, so re-running it is inert by construction.
    """
    cur = conn.cursor()
