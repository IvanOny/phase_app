...
CREATE TABLE phases (
    phase_id INTEGER PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
...
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.session_date < (SELECT start_date FROM phases WHERE phase_id = NEW.phase_id)
       OR NEW.session_date > (SELECT end_date FROM phases WHERE phase_id = NEW.phase_id) THEN
...
FOR EACH ROW EXECUTE FUNCTION check_session_date_in_phase();
...
CREATE TABLE session_exercises (
    session_exercise_id INTEGER PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
...
CREATE TABLE exercise_sets (
    exercise_set_id INTEGER PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
...
CREATE TABLE phase_aggregations (
    phase_aggregation_id INTEGER PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
